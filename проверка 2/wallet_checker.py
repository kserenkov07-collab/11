...
class WalletChecker:
    ...
    def generate_mnemonics_batch(self, count):
        ...
        logger.info(f"Сгенерировано {len(batch)} случайных мнемонических фраз")
        return batch
        ...
        try:
            ...
            if currency == "BTC":
                ...
                if response.status_code == 200:
                    ...
                    logger.debug(f"Баланс BTC: {balance}")
                    return balance

            elif currency == "ETH":
                # Используем backup URL без API ключа
                url = Config.CRYPTOCURRENCIES[currency]["api_url_backup"].format(address)
                response = requests.get(url, timeout=Config.PRIMARY_CHECK_TIMEOUT)
                if response.status_code == 200:
                    data = response.json()
                    balance = data.get('balance', 0) / 10**8  # BlockCypher возвращает баланс в satoshi
                    logger.debug(f"Баланс ETH: {balance}")
                    return balance
    ...
    def enhanced_prefilter_batch(self, batch):
        ...
        for lang, mnemonic in batch:
            ...
            try:
                ...
                if total_score >= Config.PREFILTER_THRESHOLD:
    ...
    def check_wallet_optimized(self, wallet_data):
        ...
        try:
            ...
            logger.debug(f"Сгенерированы адреса - ETH: {eth_address}, BTC: {btc_address}")
            ...
            # Первый этап: проверяем только BTC и ETH
            ...
            logger.debug(f"Первичная проверка: BTC={btc_balance}, ETH={eth_balance}, Total USD={primary_usd_total}")
            
            # Если баланс BTC и ETH не превышает порог, пропускаем кошелек
    ...
    def process_batch_optimized(self, batch):
        ...
        logger.debug("Начинаем пакетную проверку кошельков")
